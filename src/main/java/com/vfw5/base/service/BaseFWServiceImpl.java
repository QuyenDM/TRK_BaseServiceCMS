...
//    @Override
//    public List<TDTO> updateByConditionBean(List<ConditionBean> lstConditionBean) {
//        return convertListModeltoDTO(
//                gettDAO().find(tModel.getModelName(),
//                        lstConditionBean);
//    }
    ...
    @Override
    public ResultDTO createObject(TDTO tForm, Session session) {
        String id = gettDAO().saveObject(tForm.toModel(), session);
        ResultDTO result = new ResultDTO();
        try {
            Long.parseLong(id);
            result.setId(id);
    ...
    public String getSysDateExtend(String pattern, int addDay) {
        String result = "";
        try {
            result = gettDAO().getSysDateExtend(pattern, addDay);
        } catch (Exception ex) {
            Logger.getLogger(BaseFWServiceImpl.class.getName()).log(Level.SEVERE, null, ex);
        }
        return result;
    ...
    public String delete(Long id, Session session) {
        ...
        return gettDAO().delete(tModel.getModelName(), lstCondition, session);
    }

    @Override
    public String update(TDTO tDTO, Session session) {
        return gettDAO().update(tDTO.toModel(), session);
    }
